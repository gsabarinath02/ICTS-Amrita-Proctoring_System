...
class RoomClient {
  constructor(
    ...
    this.debug = false;
    this.debug
      ? window.localStorage.setItem("debug", "mediasoup*")
  ...
  getScreenEncoding() {
    ...
    if (this.enableSharingLayers) {
      ...
      console.log("SCREEN ENCODING: first codec available", {
        firstVideoCodec: firstVideoCodec,
      });
  ...
  handleHideMe() {
    ...
    if (isHideMeActive && this.isVideoPinned && myVideoPinBtn)
      myVideoPinBtn.click();
    if (isHideMeActive && this.isVideoPinned && myScreenPinBtn)
      myScreenPinBtn.click();
    if (myVideoWrap)
      myVideoWrap.style.display = isHideMeActive ? "none" : "block";
    if (myVideoWrapOff)
      myVideoWrapOff.style.display = isHideMeActive ? "none" : "block";
  ...
  async getConsumeStream(producerId, peer_id, type) {
    ...
    const codecOptions = {};
    const streamId =
      peer_id + (type == mediaType.screen ? "-screen-sharing" : "-mic-webcam");
    ...
    if (btnTs && videoPlayer) {
      btnTs.addEventListener("click", () => {
        if (videoPlayer.classList.contains("videoCircle")) {
          return this.userLog(
            "info",
            "SnapShoot not allowed if video on privacy mode",
            "top-end"
          );
        }
        this.sound("snapshot");
    ...
    this.appendMessage(
      ...
      `${icons.fileReceive} File receive: 
            ...
            <ul>
                ...
                <li>Size: ${this.bytesToSize(
                  this.incomingFileInfo.fileSize
                )}</li>
            </ul>`,
      "all",
      "all"
  ...
  roomStatus(action) {
    switch (action) {
      case "lock":
        ...
        this.userLog(
          "info",
          `${icons.lock} LOCKED the room by the password`,
          "top-end"
        );
        break;
      case "unlock":
  ...
  roomMessage(action, active = false) {
    ...
    switch (action) {
      case "pitchBar":
        ...
        this.userLog(
          "info",
          `${icons.pitchBar} Audio pitch bar ${status}`,
          "top-end"
        );
        break;
      case "sounds":
        ...
        this.userLog(
          "info",
          `${icons.sounds} Sounds notification ${status}`,
          "top-end"
        );
        break;
      default:
